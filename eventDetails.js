...
const getparams = () => {
  const postId = new URLSearchParams(window.location.search).get("eventId");
  ...
  fetch(`http://127.0.0.1:8000/event/list/${postId}`)
    .then((res) => res.json())
    .then((data) => displayDetails(data))
    .catch((err) => console.error("Error fetching post details:", err));

  fetch(`http://127.0.0.1:8000/event/comments/?event_id=${postId}`)
    .then((res) => res.json())
...
const handleReviewSubmission = (event) => {
  ...
  fetch("http://127.0.0.1:8000/event/comments/", {
    method: "POST",
    headers: {
